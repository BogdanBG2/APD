...
int* get_dst(int rank, int numProcs, int leader) {
	...
	// TODO2: Pentru fiecare proces vecin care nu este parintele procesului curent, voi trimite vectorul de parinti propriu. 

	// TODO2: Vom astepta de la fiecare proces vecin care nu este parintele procesului curent vectorul de parinti
	// si actualizam vectorul propriu de parinti daca exista informatii aditionale
	...
	for (int k = 0; k < CONVERGENCE_COEF; ++k) {
		...
		*/
		for (int i = 0; i < num_neigh; ++i) {
			...
			MPI_Recv(&leader_neigh, 1, MPI_INT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
